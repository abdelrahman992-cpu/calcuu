...
class MatrixUtil {
  static String mtoString({required String name, List<List<num>>? list}) {
  ...
  static num getDetValue(List<List<num>> list) {
    ...
    var result = 0.0;
    int num = list.length;
    var nums = List.filled(num, 0, growable: false);
    for (int i = 0; i < list.length; i++) {
      if (i % 2 == 0) {
        nums[i] = (list[0][i] * getDetValue(getConfactor(list, 0, i))).toInt();
      } else {
        nums[i] = (-list[0][i] * getDetValue(getConfactor(list, 0, i))).toInt();
      }
    }
  ...
  static List<List<num>> upperTriangular(List<List<num>> list) {
    ...
    var divisor =
        new List.filled(list.length, null, growable: false); //用于记录对角线下的每一行的倍数
    for (int i = 0; i < newlist.length; i++) {
      ...
      if (max == 0)
        throw FormatException(SettingData.language == "zh"
            ? "第 $i 列全为零！"
            : "The $i column of the matrix is all zero");
      for (int k = i + 1; k < newlist.length; k++) {
        if (newlist[k][i] == 0) {
          continue;
        }
        divisor[k] = (newlist[k][i] / newlist[i][i]) as Null; //存储k行与i行首位的倍数
        newlist[k] =
