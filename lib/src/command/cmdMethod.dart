...
import 'package:calculator/src/data/userData.dart';
  ...
  static num factorial(num d) {
    ...
    if (d < 0) {
      if (SettingData.language == 'zh')
        ...
        throw FormatException('Negative Numbers have no factorial');
    }
    if (d == 0) {
      return 1.0;
    }
  ...
  static Future<num> _calculus(UserFunction fx, num a, num b) async {
    ...
    if (fx.paras.length != 1) {
      if (SettingData.language == 'zh')
        ...
        throw FormatException(
            'The parameter of the integrated function is only allowed to be one');
    }
    if (a > b) {
      var temp = b;
      b = a;
  ...
  static num _getValue(String valStr) {
    ...
    try {
      ...
      if (SettingData.language == 'zh')
        ...
        throw FormatException(
            'Integral functions can only return real Numbers');
    }
    return result;
  }
  ...
  static List<List<num>>? polyomial(List<List<num>> list) {
    if (list.length != 1) {
      if (SettingData.language == 'zh')
        ...
        throw FormatException(
            'Polynomial solution function parameters should be a single row matrix');
    }
