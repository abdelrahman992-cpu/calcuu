  ...
  String _handleLineEquations(String euationsStr) {
    ...
    try {
      for (int i = 0; i < equations.length; i++) {
        ...
        if (length != ns.length) {
          if (SettingData.language == 'zh')
            return '第 ${i + 1} 行参数数量与第一行不一致';
          else
            return 'The number of arguments in line ${i + 1} is inconsistent with that in line 1';
        }
      ...
      if (SettingData.language == 'zh')
        ...
        return 'Coefficient matrix input error';
    }
    ...
    for (int i = 0; i < resultList.length; i++) {
      sb.write(SettingData.language == 'zh'
          ? '第 ${i + 1} 个解为:   '
          : 'The ${i + 1} solution is:  ' +
              resultList[i][0].toStringAsFixed(SettingData.fixedNum.round()));
      sb.write('\n');
    }
    result = sb.toString();
    postMatrix = [];
  ...
  String _nonlinearEquation(String equation) {
    ...
    for (var n in nums) {
      ...
      if (temp != null) {
        ...
        return SettingData.language == 'zh'
            ? "$n 不能被识别为数字"
            : '$n cannot be identified as a number';
      }
    }
    ...
    for (var l in resultMatrix!) {
      sb.write(SettingData.language == 'zh'
          ? '第$index 个解为： '
          : 'The $index solution is:  ' +
              '${l[0].toStringAsFixed(SettingData.fixedNum.round())}');
      sb.write('\n');
      index++;
    }
    return sb.toString();
