...
import 'package:calculator/src/command/cmdMethod.dart';
import 'package:calculator/src/command/matrix.dart';
import 'package:calculator/src/data/dbUtil.dart';
import 'package:calculator/src/data/settingData.dart';
import 'package:flutter/cupertino.dart';

class UserData with ChangeNotifier {
  UserData() {
    Map<String, num> dbs = <String, num>{};
    Map<String, List<List<num>>> matrixs = <String, List<List<num>>>{};
    userFunctions = <UserFunction>[];
  }
  late Map<String, num> dbs; //存储浮点数变量
  late Map<String, List<List<num>>> matrixs; //存储矩阵变量
  late List<UserFunction> userFunctions; //存储用户自定义函数
  ...
  Future<String> handleCommand(String command) async {
    ...
    if (inputMatrix.firstMatch(cmd) != null) {
      ...
      if (SettingData.language == 'zh')
        result = "$command  为未知命令";
      else
        result = "$command  is unknown command";
    }
    return result;
  }
  ...
  String _handleGetMatrix(String cmd) {
    late String name; //变量名
    late int vertical; //行数， 列数
        ...
        for (String numraw in numraws) {
          ...
          if (dnum != null) {
            ...
            if (SettingData.language == 'zh')
              ...
              return "illegal character:  $numraw";
          }
        }
      } else {
        if (SettingData.language == 'zh')
          ...
          return "The number of rows in the matrix is inconsistent";
      }
      index++;
    }
    addMatrix(name, list);
      ...
      if (name != null) {
        if (re is List<List<num>>) {
          ...
          if (SettingData.language == 'zh')
            ...
            result = "This type cannot be stored";
        }
      } else {
        if (re is List<List<num>>) {
          result = MatrixUtil.mtoString(list: re, name: '');
        } else if (re is num) {
      ...
      case 'inv':
        if (vals.length != 1 || vals[0] is! List<List<num>>) {
          if (SettingData.language == 'zh')
            ...
            throw FormatException('Function parameter transfer error: inv');
        }
      ...
      case 'tran':
        if (vals.length != 1 || vals[0] is! List<List<num>>) {
          if (SettingData.language == 'zh')
            ...
            throw FormatException('Function parameter transfer error: tran');
        }
      ...
      case 'value':
        if (vals.length != 1 || vals[0] is! List<List<num>>) {
          if (SettingData.language == 'zh')
            ...
            throw FormatException('Function parameter transfer error: value');
        }
        ...
        if (vals.length != 3 ||
          ...
          if (SettingData.language == 'zh')
            ...
            throw FormatException(
                'Function parameter transfer error: lagrange');
        }
      ...
      case 'sum':
        if (vals.length != 1 || vals[0] is! List<List<num>>) {
          if (SettingData.language == 'zh')
            ...
            throw FormatException('Function parameter transfer error: sum');
        }
      ...
      case 'absSum':
        if (vals.length != 1 || vals[0] is! List<List<num>>) {
          if (SettingData.language == 'zh')
            ...
            throw FormatException('Function parameter transfer error: absSum');
        }
      ...
      case 'average':
        if (vals.length != 1 || vals[0] is! List<List<num>>) {
          if (SettingData.language == 'zh')
            ...
            throw FormatException('Function parameter transfer error: average');
        }
      ...
      case 'absAverage':
        if (vals.length != 1 || vals[0] is! List<List<num>>) {
          if (SettingData.language == 'zh')
            ...
            throw FormatException(
                'Function parameter transfer error: absAverage');
        }
  ...
  Future<dynamic> handleCalcuStr(String caculStr) async {
        ...
        for (int i = 0; i < caculStrs.length; i++) {
          ...
          if (inlayMethod.hasMatch(caculStr)) {
            ...
            caculStr = caculStr.replaceFirst(
                RegExp(inlayMethod.firstMatch(caculStr)!.group(0).toString()),
                'caculStrTemp$i');
    ...
    for (Match m in opers) {
      operStrs.add(m.group(0).toString());
    }
    //运算数列表
    List nums = [];
...
///内置的命令行函数
class CmdMethod {
  late String name; //函数名
  late String ename; //函数的英文名
  late String cmdText; //函数命令
  late String emethodDescription; //函数的英文详细描述
  late String methodDescription; //函数详细描述
