  ...
  static Map<String, List<List<num>>> readMatrix() {
    ...
    getDB().then((db) {
      db.rawQuery('select * from Matrixs').then((list) {
        list.forEach((m) {
          matrixs[m['name'] as String] = _stringToList(m['value'] as String);
        });
      });
    });
    return matrixs;
  ...
  static List<UserFunction> readUserFun() {
    ...
    getDB().then((db) {
      db.rawQuery('select * from UserFunction').then((list) {
        list.forEach((m) {
          String funName = m['funName'] as String;
          List<String> paras = (m['paras'] as String)
              .substring(1, (m['paras'] as String).length - 1)
              .replaceAll(' ', '')
              .split(',');
          List<String> funCmds = (m['funCmds'] as String)
              .substring(1, (m['funCmds'] as String).length - 1)
              .split(',');
  ...
  static Map<String, num> readNum() {
    ...
    getDB().then((db) {
      db.rawQuery('SELECT * FROM Nums').then((list) {
        list.forEach((m) {
          dbs[m['name'] as String] = num.parse(m['value'].toString());
        });
      });
    });
    return dbs;
